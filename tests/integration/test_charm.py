...
from tests.integration.helpers import get_k8s_service_clusterip
...
async def test_kiali_is_available(ops_test: OpsTest):
    ...
    kiali_service_ip = get_k8s_service_clusterip(ops_test.model.name, APP_NAME)

    # Assert that Kiali is available via the charm's service
    assert kiali_service_ip is not None, "Kiali service IP not found"
    resp = requests.get(url=f"http://{kiali_service_ip}:20001/{ops_test.model.name}-{APP_NAME}")
